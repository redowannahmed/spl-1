...
/*public class Main {
 ...
 private static void showAdminMenu(Scanner sc, WalletManager walletManager) {
     MenuManager menuManager = new MenuManager(); // Add the MenuManager instance
     ...
     while (true) {
         ...
         switch (choice) {
             ...
             case 2: // View Menu
                 UI.clearScreen();
                 menuManager.viewMenu(); // Call viewMenu method
                 UI.waitForUserInput("Press enter to go back to main panel", sc);
                 break;

             case 3: // Update Menu
                 UI.clearScreen();
                 menuManager.updateMenu(sc); // Call updateMenu method
                 UI.waitForUserInput("Press enter to go back to main panel", sc);
                 break;
...
public class Main {
    ...
    private static void showAdminMenu(Scanner sc, WalletManager walletManager, MenuManager menuManager) {
        while (true) {
            UI.clearScreen();
            String[] adminMenuOptions = {"View and Process Recharge Requests", "View Menu", "Update Menu", "Logout"};
            UI.printBoxedMenu(adminMenuOptions, "Admin Dashboard");
